...
eth0 にかかる通信の内容(通信量)を測定するコードを rust で書いてください

ローカル IP ごとに rx と tx を prometheus(localhostl9090)に出力するコードを書いてください

```
  - job_name: "lcoalpacketdump"
    scrape_interval: 1s
    static_configs:
      - targets: ["localhost:59122"]
```

`network_ip_tx_bps`,`network_ip_rx_bps`という項目で
local_ip,nic を含めて 1 秒間の通信量を bps で出力してください

また
`network_ip_tx_bps_total`,`network_ip_rx_bps_total`という項目で
nic の 1 秒間の合計通信量を bps で出力してください

nic と local_ip との割り当ては

```curl "http://localhost:32599/status"
{"config":{"lan":"eth2","wan0":"eth0","wan1":"eth1"},"mappings":{"10.40.0.3":"wan1"}}
```

このような形でアクセスするとどの IP がどの wan に破り当たっているか
どの wan はどの nic なのかが返ってきます
mappings にない IP は全て wan0 に破り当たっています。
